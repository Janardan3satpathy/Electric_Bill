...
def admin_dashboard(user_details):
    ...
    # REORGANIZED TABS
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["💰 Dues & Payments", "👥 Tenants & Flats", "🏢 Meters", "⚡ Generate Monthly", "📊 Records"])

    # --- TAB 1: DUES & PAYMENTS (THE NEW FEATURE) ---
    with tab1:
        ...
        if not user_opts:
            ...
            # DROPDOWN: Which months are pending?
            ...
            with st.form("update_tenant_full"):
                ...
                if st.form_submit_button("Update Tenant"):
                    conn.table("profiles").update({
                        ...
                        "mobile": new_mobile,
                        "rent_amount": new_rent # Saving Rent
    ...
    with tab3:
        ...
        # ... (Identical logic for Floor Submeters) ...
        # Note: Collapsed to save space as logic is same as previous, just ensure variables are correct
    ...
    # --- TAB 4: GENERATE BILLS (UPDATED FOR RENT) ---
    with tab4:
        st.subheader("Generate Monthly Bills (Elec + Rent)")
        
        col_gen1, col_gen2 = st.columns(2)
        gen_date = col_gen1.date_input("Bill Date for Generation", value=date.today())
        
        # ... (Rates Fetching Logic same as before) ...
        rates_data = {}
        ...
        if not rates_data:
            st.warning("⚠️ Main Meters not saved for this month.")
        else:
            ...
            st.markdown("### 📋 Tenant Preview")
            
            bill_batch = [] # For Electricity
            rent_batch = [] # For Rent

            if users_resp.data:
                for t in users_resp.data:
                    with st.expander(f"🧾 {t.get('full_name')} (Flat: {t.get('flat_number')})"):
                        ...
                        col_d1.write(f"**⚡ Electricity:** ₹{total_elec_amt}")
                        col_d2.write(f"**🏠 Rent:** ₹{rent_amt}")
                        col_d3.metric("Total for Month", f"₹{total_elec_amt + rent_amt}")
                        ...
                        if t_curr > 0 or water_cost > 0:
                            bill_batch.append(elec_obj)
                            rent_batch.append(rent_obj)
                            
                            if st.button(f"Generate for {t['full_name']}", key=f"btn_{t['id']}"):
                                conn.table("bills").upsert(elec_obj, on_conflict="user_id, bill_month").execute()
                                conn.table("rent_records").upsert(rent_obj, on_conflict="user_id, bill_month").execute()
                                st.success("Bills Generated!")

            if st.button("🚀 GENERATE ALL (Rent + Elec)", type="primary"):
                if bill_batch:
                    for i in range(len(bill_batch)):
                        conn.table("bills").upsert(bill_batch[i], on_conflict="user_id, bill_month").execute()
                        conn.table("rent_records").upsert(rent_batch[i], on_conflict="user_id, bill_month").execute()
                    st.balloons()
                    st.success(f"✅ Generated {len(bill_batch)} Electricity bills and Rent records!")
    ...
    with tab5:
        ...
        if r_opt == "Electricity Bills":
            ...
            res = conn.table("rent_records").select("*, profiles(full_name)").order("created_at", desc=True).limit(20).execute()
            if res.data:
                # Flatten the 'profiles' nested dict to get name
                data = []
                for r in res.data:
                    r['name'] = r['profiles']['full_name'] if r['profiles'] else 'Unknown'
                    del r['profiles']
                    data.append(r)
