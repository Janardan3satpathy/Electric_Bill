...
def set_background():
    # Adds a dynamic background image (Point 11 from your notes)
    st.markdown(
        """
        <style>
        .stApp {
            background-image: linear-gradient(rgba(255,255,255,0.9), rgba(255,255,255,0.9)), url("https://images.unsplash.com/photo-1560518883-ce09059eeffa?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80");
...
def perform_logout():
    ...
    st.toast("Thank you for using PropEase! Visit again soon. 👋", icon="👋")
...
def admin_dashboard(user_details):
    ...
    if properties:
        ...
        st.warning("No properties found. Please add a property in the 'Properties' tab to start.")
        active_prop_id = None
    st.divider()
    ...
    with tab1:
        ...
        pending_approvals = conn.table("bills").select("*").eq("status", "Verifying").eq("property_id", active_prop_id).execute() if active_prop_id else None
        rent_approvals = conn.table("rent_records").select("*, profiles(full_name)").eq("status", "Verifying").eq("property_id", active_prop_id).execute() if active_prop_id else None
    ...
    with tab2:
        ...
        if active_prop_id and users_resp.data:
    ...
    with tab3:
        if active_prop_id:
            ...
            main_prev = 0
            try:
                last_meter = conn.table("main_meters").select("current_reading").eq("meter_name", meter_type).eq("property_id", active_prop_id).lt("bill_month", str(bill_date)).order("bill_month", desc=True).limit(1).execute()
            ...
            if meter_type == "Ground Meter":
                ...
                p101 = get_last_month_reading("101", bill_date, active_prop_id)
                ...
                p102 = get_last_month_reading("102", bill_date, active_prop_id)
                ...
                p201 = get_last_month_reading("201", bill_date, active_prop_id)
                ...
                p401 = get_last_month_reading("401", bill_date, active_prop_id)
                ...
                p302 = get_last_month_reading("302", bill_date, active_prop_id)
    ...
    with tab4:
        if active_prop_id:
            ...
            admin_paid = 0
            mm_res = conn.table("main_meters").select("total_bill_amount").eq("bill_month", str(gen_date)).eq("property_id", active_prop_id).execute()
            if mm_res.data:
            ...
            with col_A:
                ...
                if not rates_data:
                    ...
                    sub_res_all = conn.table("sub_meter_readings").select("*").eq("bill_month", str(gen_date)).eq("property_id", active_prop_id).execute()
                    ...
                    if st.button("🚀 Generate Electricity Bills", type="primary", disabled=len(elec_batch)==0):
                        if elec_batch:
                            for obj in elec_batch:
                                conn.table("bills").upsert(obj, on_conflict="property_id, user_id, bill_month").execute()
                ...
                if st.button("🚀 Generate Rent Bills", type="primary"):
                    if rent_batch:
                        for obj in rent_batch:
                            conn.table("rent_records").upsert(obj, on_conflict="property_id, user_id, bill_month").execute()
    ...
    with tab5:
        if active_prop_id:
            ...
            if r_opt == "Electricity Bills":
                ...
                rent_res = conn.table("rent_records").select("*").eq("property_id", active_prop_id).order("created_at", desc=True).limit(20).execute()
                if rent_res.data:
    ...
    # --- TAB 6: OUTSTANDING SUMMARY (WITH CSV EXPORT) ---
    with tab6:
        if active_prop_id:
            st.subheader("📉 Consolidated Outstanding Summary")
            
            summary_data = []
            all_tenants = conn.table("profiles").select("*").eq("role", "tenant").eq("property_id", active_prop_id).execute()
            all_pending_elec = conn.table("bills").select("*").eq("property_id", active_prop_id).neq("status", "Paid").execute()
            all_pending_rent = conn.table("rent_records").select("*").eq("property_id", active_prop_id).neq("status", "Paid").execute()
            ...
            if summary_data:
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f'outstanding_summary_{selected_prop_name}_{date.today()}.csv',
                    mime='text/csv',
                )
            else:
                st.info("No tenant data available.")

    # --- TAB 7: MANAGE PROPERTIES (NEW) ---
    with tab7:
        st.subheader("⚙️ Add & Manage Properties")
        with st.form("add_prop_form"):
            ...
            if st.form_submit_button("Create Property"):
                try:
                    ...
                    st.success("Property created successfully! Please refresh.")
                    st.rerun()
                except Exception as e:
                    st.error(f"Error creating property: {e}")

    # --- TAB 8: FEEDBACK FORM (NEW) ---
    with tab8:
        st.subheader("💬 Support & Feedback")
        st.write("Found a bug or have a feature request? Let the developer know.")
        with st.form("feedback_form"):
            feedback_msg = st.text_area("Your message:")
            if st.form_submit_button("Submit Feedback"):
                try:
                    conn.table("feedback").insert({
                        "owner_id": user_details['id'],
...
def tenant_dashboard(user_details):
    ...
    if total_due > 0:
        ...
        with st.expander("💳 PAY DUES (UPI)", expanded=True):
            upi_id = "propease@upi" # Updated to PropEase
            upi_url = f"upi://pay?pa={upi_id}&pn=PropEase_Society&am={total_due}&cu=INR"
